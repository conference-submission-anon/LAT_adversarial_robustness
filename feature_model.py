...
class FeatureModel(object):
    ...
    def __init__(self, mode, scope = '', reuse = False, placeholder = 0):
        """ResNet constructor.
    ...
    def make_placeholder(self, shape, dtype = tf.float32) :
        return tf.placeholder(shape = shape, dtype = dtype)    

    def check_placeholder(self, x) :
        self.nets.append(x)
        if len(self.nets) == self.placeholder :
            self.out = x
            self.x_placeholder = self.make_placeholder(shape = x.get_shape().as_list(), dtype = tf.float32)
    ...
    def _build_model(self):
        ...
        with tf.variable_scope(self.scope, reuse = self.reuse) :
            ...
            if self.placeholder == 0 :
                self.x_placeholder = self.x_input
                self.out = self.pre_softmax
